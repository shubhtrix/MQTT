...
void on_connect (struct mosquitto *clnt_isnt, void *obj, int ret) {
...
void on_message (struct mosquitto *clnt_inst, void *obj, const struct mosquitto_message *msg) {
	...
	printf("Topic :: %s Message Length :: %d\nReceived Message :: %s\n", 
	...
	if (msg->payloadlen) {
		...
		//printf("Final Data to post to clixon : %s\n", post_obj);	
		//strcpy(post_obj,msg->payload);
		//printf("Final Data to post to clixon : %s\n", post_obj);	
		sprintf(post_obj,"curl -X PUT -d '%s' http://127.0.0.1/restconf/data", (char *)msg->payload);	
		printf("Final Data to post to clixon : %s\n", post_obj);	
		printf("Final Data Length : %ld\n", strlen(post_obj));	
		syslog (LOG_INFO, "POSTED DATA :: %s", post_obj);
		
		printf("_____________________________________________________________________\n");	

		//mosquitto_publish ( clnt_inst, NULL, id, 21, "TEST STRING!!!", 0, true);
		//mosquitto_publish ( clnt_inst, NULL, id, strlen(string), string, 0, true);
		...
		//mosquitto_publish (clnt_inst, mid, "NETCONF", (int)strlen(post_obj), "TEST STRING!!!", 0, true);
	}
}
